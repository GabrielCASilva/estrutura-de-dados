...
void queue_resize(
    Queue *queue); // redimensiona a fila para o doubro do tamanho original
int queue_is_null(Queue *queue);  // verifica se a fila é nula
int queue_is_empty(Queue *queue); // verifica se a fila está vazia
int queue_is_null_and_is_empty(
    Queue *queue);              // verifica se a fila é nula e se está vazia
void queue_print(Queue *queue); // prints the stack
